<?php
	...
	$load = 25;
	getFeeds($_SESSION['user_id'],$load); 
...
<?php if ($shared !== 0){?>
<?php echo SHARED ?> <a href="<?php echo ISVIPI_URL.'profile/' ?><?php getUserDetails($shared); echo $username;?>"><?php echo $username; ?>'s</a> <?php echo STATUS ?>
<?php } ?>
...
<?php } ?>
<div class="feed_option_notices pull-right">
...
$( "textarea" ).on({
	mouseenter: function() {
	$('.timeline_div').timer('stop');
	},
	mouseleave: function() {
	$('.timeline_div').timer('start');
	},
	click: function() {
	$('.timeline_div').timer('stop');
	}
...
<!------ share user status -->
                 <div style="display: none" id="shareUserStatus<?php echo $FIDentinty ?>">
